...
%                formed_file_name = strcat('fitsL_',file_dict(start).name,'_',mc{size(mc,2)},'.mat');
               formed_file_name = strcat(name2save,'_fitsL',file_dict(start).name,'_',mc{size(mc,2)},'.mat');
           ...
           for r = 2:6
               try
                   formed_file_name = strcat(name2save,'_fitsL',file_dict(start).name,'_r_',num2str(r),'.mat');
                   obj = load(formed_file_name);
                   if(sum(isnan(obj.final_imputed(:)))==0)
    ...
    clear dataX;
    %disp('start2');
    matrixFormation(m,n,file_dict,matrixUsage,corrStruct,name2save,total,csv_or_tab);
    %toc
...
function dropDone = dropAllInternalFile(name2save)
    dropDone = 0;
    files = dir(strcat(name2save,'_fitsL_*.mat'));
...
function matrixFormation(m,n,mtree,matrixUsage,corrStruct,name2save,total,csv_or_tab)
    newMatrix = zeros(m,n);
    for i = 1: total
    ...
    if csv_or_tab ~= 'tab' 
    	...
    	writematrix(newMatrix',strcat(name2save,'.txt'),'Delimiter','tab');
    end
end
